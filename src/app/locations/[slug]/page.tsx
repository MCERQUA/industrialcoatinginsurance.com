...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${loc.intro} General liability, contractors pollution liability, workers' comp, commercial auto, and equipment coverage for ${loc.name} industrial coating contractors. 15-minute quotes.`,
  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              <span className="eyebrow"><span className="h-px w-8 bg-clay" />Coverage for {loc.name} industrial coating contractors</span>
              <h2 className="mt-3 h-section">The full program, built for {loc.name} industrial coating contractors.</h2>
              <p className="mt-4 lead">From a single crew to a multi-site operation, we coordinate every line a {loc.name} industrial coating contractor needs.</p>
            </FadeIn>
        ...
        <CTABand title={`Insuring industrial coating contractors in ${loc.name} since 2005`} description={`Local knowledge, A-rated markets, and 15-minute quotes. Call ${SITE.phone} or request a quote online.`} />
      </main>
      <Footer />
    </>
  );
